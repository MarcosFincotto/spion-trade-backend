...
export class Dayprofit {
  private static readonly TIMEOUT = 30 * 1000;

  private static readonly HOST = 'trade.dayprofitinvest.com';
  ...
  public async connect(): Promise<boolean> {
    ...
    const timeout = new Promise<void>((resolve) => {
      timeoutId = setTimeout(resolve, Dayprofit.TIMEOUT * 1000);
    });

    const connection = new Promise<void>((resolve) => {
      this.socket = new WebSocket(Dayprofit.WWS_URL);
  ...
  public async establishConnection(): Promise<boolean> {
    ...
    await this.event.waitForEvent<number>('timeSync', Dayprofit.TIMEOUT);
    ...
    const auth = await Dayprofit.authenticate(this.email, this.password);
  ...
  public async buyAndCheckWin({
    ...
    const response = await this.event.waitForEvent<BuyAndCheckWinResponse>(
      ...
      duration * 60 * 1000 + Dayprofit.TIMEOUT,
      () => {},
