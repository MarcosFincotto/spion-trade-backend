...
export class IQOption {
  private static readonly TIMEOUT = 30 * 1000;

  private static readonly HOST = 'iqoption.com';

  private static readonly HTTPS_URL = `https://auth.${this.HOST}/api/v2`;
  ...
  public async connect(): Promise<boolean> {
    ...
    const timeout = new Promise<void>((resolve) => {
      timeoutId = setTimeout(resolve, IQOption.TIMEOUT * 1000);
    });

    const connection = new Promise<void>((resolve) => {
      this.socket = new WebSocket(IQOption.WWS_URL);
  ...
  public async establishConnection(): Promise<boolean> {
    ...
    await this.event.waitForEvent<number>('timeSync', IQOption.TIMEOUT);
    ...
    const auth = await IQOption.authenticate(this.email, this.password);
  ...
  public async buyAndCheckWin({
    ...
    const response = await this.event.waitForEvent<BuyAndCheckWinResponse>(
      ...
      duration * 60 * 1000 + IQOption.TIMEOUT,
      () => {},
