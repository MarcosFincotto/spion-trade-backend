...
export class BullexBinary {
  ...
  public async connect(): Promise<boolean> {
    ...
    const timeout = new Promise<void>((resolve) => {
      timeoutId = setTimeout(resolve, BullexBinary.TIMEOUT * 1000);
    });

    const connection = new Promise<void>((resolve) => {
      this.socket = new WebSocket(BullexBinary.WWS_URL);
  ...
  public async establishConnection(): Promise<boolean> {
    ...
    await this.event.waitForEvent<number>('timeSync', BullexBinary.TIMEOUT);
    ...
    const auth = await BullexBinary.authenticate(this.email, this.password);
  ...
  public async buyAndCheckWin({
    ...
    const response = await this.event.waitForEvent<BuyAndCheckWinResponse>(
      ...
      duration * 60 * 1000 + BullexBinary.TIMEOUT,
      () => {},
